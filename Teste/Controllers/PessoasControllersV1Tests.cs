...
public class PessoaControllerV1Tests
{
    ...
    {
        var pessoas = new List<PessoaV1Dto> { new PessoaV1Dto { Id = 1, Nome = "Miguel" } };
        _mockService.Setup(s => s.GetAllAsync()).ReturnsAsync(pessoas);

        var result = await _controller.GetAll();

        var okResult = Assert.IsType<OkObjectResult>(result.Result);
        var lista = Assert.IsAssignableFrom<IEnumerable<PessoaV1Dto>>(okResult.Value);
        Assert.Single(lista);
    }
    ...
    {
        var pessoa = new PessoaV1Dto { Id = 1, Nome = "João" };
        _mockService.Setup(s => s.GetByIdAsync(1)).ReturnsAsync(pessoa);

        var result = await _controller.GetById(1);

        var okResult = Assert.IsType<OkObjectResult>(result.Result);
        var dto = Assert.IsType<PessoaV1Dto>(okResult.Value);
    ...
    {
        _mockService.Setup(s => s.GetByIdAsync(99)).ThrowsAsync(new KeyNotFoundException("Pessoa não encontrada"));
    ...
    {
        var dto = new PessoaV1Dto { Nome = "Maria" };
    ...
    {
        var dto = new PessoaV1Dto { Id = 1, Nome = "Miguel Atualizado" };
        _mockService.Setup(s => s.UpdateAsync(1, dto)).ReturnsAsync(dto);

        var result = await _controller.Update(1, dto);

        var okResult = Assert.IsType<OkObjectResult>(result.Result);
        var pessoaAtualizada = Assert.IsType<PessoaV1Dto>(okResult.Value);
    ...
    {
        var dto = new PessoaV1Dto { Nome = "Inexistente" };
    ...
    {
        var dto = new PessoaV1Dto { Nome = "Teste" };
